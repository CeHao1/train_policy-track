...
class Roadster:
    ...
    def step(self, action):
        ...
        self.Vx     += dt * (acc - (Fyf*np.sin(delta))/m + dpsi*Vy - w_wind*Vx**2)
        self.Vy     += dt * ((Fyf+np.cos(delta) + Fyr)/m - dpsi*Vx)
        self.dpsi   += dt * ((lf*Fyf*np.cos(delta) - lr*Fyr)/Iz)

        state = np.array([self.s, self.epsi, self.ey, self.Vx, self.Vy, self.dpsi])
        
        # done
        done = bool(self.s >= self.map.s[-1] or
                    self.epsi <= self.epsi_limit[0] or
                    self.epsi >= self.epsi_limit[1] or
                    self.ey <= self.ey_limit[0] or
